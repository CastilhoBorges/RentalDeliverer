    ...
    public class DelivererDateEndAndPriceService(ApplicationDbContext context)
    {
        ...
        { 
            var rental = await _context.Rentals
                .FindAsync(id)
                ?? throw new Exception("aluguel não existe");

            var rentalType = await _context.RentalTypes.FindAsync(rental.RentalTypeId)
        ...
        private decimal GetPenaltyPercent(int rentalDays)
        {
            return rentalDays switch
            {
                ...
                45 => 80,
                50 => 100,
                _ => throw new Exception("paro aqui"),
            };
        }
    }
}
